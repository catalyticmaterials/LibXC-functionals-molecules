...
def setup_calculator(atoms, functional, correction):
    ...
    if formula in magmom_map:
        atoms.set_initial_magnetic_moments(magmoms=magmom_map[formula])

    # Determine Hund's rule and spin polarization
    hund = True if formula in ['H', 'O'] else False
    spinpol = True if formula in ['O2', 'H', 'O'] else False
    nbands = -1 if formula == 'H' else -4
    ...
    calc_config = {
        ...
        'hund': hund,
        'occupations': {'name': 'fermi-dirac', 'width': 0, 'fixmagmom': True},
        'spinpol': spinpol,
        'txt': f'{formula}_{functional}_{correction}.txt',
    }
...
def run(functional, correction, ase_database):
    """
    Run calculations with given parameters starting with HSE06-optimized structures.

    Parameters:
    functional (str): The density functional to use for the calculation.
    correction (str): The correction method to apply ('SIC', 'U', or None).
    ase_database (str): Path to the ASE database to store results.

    Returns:
    None
    """
    # Connect to the ASE database
    database = db.connect(ase_database)

    for row in database.select(relaxed=True, density_functional='PBE'):
        try:
            ...
            # Print energy result
            parprint(f'{formula}_{functional}_{correction} energy: {ref_energy}')

            # Move output file to the output directory
            if world.rank == 0:
                output_file = f'{formula}_{functional}_{correction}.txt'
                shutil.move(output_file, os.path.join(OUTPUT_DIR, output_file))

            end_time = time.time()
            parprint(f'Time: {end_time - start_time:.2f} s')

            # GGA functionals list for non-self consistent energies
            df_list = [
                # GGA xc
                "GGA_XC_B97_GGA1",
                "GGA_XC_HCTH_407",
            ...
            # Calculate non-self-consistent energies for each functional
            for df in df_list:
                try:
...
def main():
    run('RPBE', '', 'DF-and-water-formation.db')
    #run_sic('GGA_X_N12+GGA_C_N12', '', 'DF-and-water-formation.db')
    #run_sic('GGA_XC_KT3', '', 'DF-and-water-formation.db')
    #run_sic('GGA_XC_VV10', '', 'DF-and-water-formation.db')
    #run_sic('PBE', '', 'DF-and-water-formation.db')
